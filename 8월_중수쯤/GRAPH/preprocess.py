...
parser.add_argument('--rootdir', type=str, default='C:\\Users\\LeeKihoon\\PycharmProjects\\Data Augmentation using GPT for Boosting Performance on NLI Tasks\\7월_리프레쉬\\그래프대회\\dataset', help='Directory to download the OGB dataset.')
...
print(dataset.paper_label.shape) # numpy array of shape (num_papers, ), storing target labels of papers.

print(dataset.paper_year[train_idx].shape)# all elements < 2019
print(dataset.paper_year[valid_idx].shape) # all elements are 2019
print(dataset.paper_year[testdev_idx].shape) # all elements are 2020
print(dataset.paper_year[testchallenge_idx].shape) # all elements are 2020
#%%
i = 0
for data in dataset.paper_label :
    print(data is None)
...
print(paper_offset-inst_offset)
dataset
#%%
g = dgl.heterograph({
...
paper_feat = dataset.paper_feat #OSError: [WinError 8] 메모리 리소스가 부족하여 이 명령을 처리할 수 없습니다
