...
class MAG240M(LightningDataModule):
    def __init__(self, batch_size: int, sizes: List[int],
    ...
    def prepare_data(self):
        ...
        if not osp.exists(path):  # Will take approximately 16 minutes...
            ...
            row, col, _ = torch.load(
                f'/paper_to_paper_symmetric.pt').coo()
            rows, cols = [row], [col]
...
if __name__ == '__main__':
    ...
    seed_everything(42)
    datamodule = MAG240M(batch_size=args.batch_size, sizes=args.sizes, in_memory=args.in_memory)
